...
namespace rythe::core::compute
{
	...
	public:
		...
		using dimension = std::variant<rsl::size_type, d2, d3>;
		...
		Kernel& global(rsl::size_type, rsl::size_type);
		Kernel& global(rsl::size_type, rsl::size_type, rsl::size_type);
		 ...
		 * @param name The name of the Kernel Argument as it appears in the Kernel.
		 */
		Kernel& setKernelArg(void* value, rsl::size_type size, const std::string& name);
		 ...
		 */
		rsl::size_type getMaxWorkSize() const;
	...
	private:
		...
		std::tuple<std::vector<rsl::size_type>, std::vector<rsl::size_type>, rsl::size_type> parse_dimensions()
		{
			rsl::size_type dim = 1;
			rsl::size_type* v;
			if ((v = reinterpret_cast<rsl::size_type*>(std::get_if<d3>(&m_global_size))))
			{
				dim = 3;
			}
			else if ((v = reinterpret_cast<rsl::size_type*>(std::get_if<d2>(&m_global_size))))
			{
				dim = 2;
			}
			else
			{
				v = &std::get<rsl::size_type>(m_global_size);
			}

			std::vector<rsl::size_type> l;
			for (rsl::size_type i = 0; i < dim; ++i)
			{
				l.emplace_back(m_local_size / dim);
			}

			return std::make_tuple(std::vector<rsl::size_type>(v, v + dim), l, dim);
		}

		dimension m_global_size;
		rsl::size_type m_local_size;
