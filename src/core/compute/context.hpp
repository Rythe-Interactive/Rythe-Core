...
namespace rythe::core::compute
{
    ...
    public:
        ...
        */
        ...
        {
            rsl::size_type width = static_cast<rsl::size_type>(img.resolution().x);
            rsl::size_type height = static_cast<rsl::size_type>(img.resolution().y);
            rsl::size_type depth = 0;
        ...
        static Buffer createImageFromOpenGLImage(rsl::uint target, rsl::uint texture, buffer_type type, std::string name = "", rsl::uint mip_level = 0)
        {
            return Buffer(instance.m_context, target, texture, mip_level, type, std::move(name));
        }

        static Buffer createImageFromOpenGLBuffer(rsl::uint bufferid, buffer_type type, std::string name = "")
        {
            return Buffer(instance.m_context, bufferid, type, false, std::move(name));
        }
        static Buffer createImageFromOpenGLRenderBuffer(rsl::uint bufferid, buffer_type type, std::string name = "")
        {
