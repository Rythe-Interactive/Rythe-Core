...
namespace rythe::core::compute {
    ...
    Program::Program(cl_context ctx, cl_device_id device, filesystem::basic_resource container /* , bool source_is_il */) {
        ...
        // -DRYTHE_LIBRARY this is indicates to your kernel that it was built for use with the ARGS-Engine, it defines the macor RYTHE_LIBRARY
        ...
        if constexpr (RYTHE_CONFIGURATION == RYTHE_DEBUG_VALUE) {
            //DEBUG
            ret = clBuildProgram(m_program, 1, &device, "-cl-std=CL2.0 -cl-kernel-arg-info -DRYTHE_LIBRARY -DDEBUG ", nullptr, nullptr);
        } else {
            //NDEBUG
            ret = clBuildProgram(m_program, 1, &device, "-cl-std=CL2.0 -cl-kernel-arg-info -DRYTHE_LIBRARY -DNDEBUG", nullptr, nullptr);
        }
