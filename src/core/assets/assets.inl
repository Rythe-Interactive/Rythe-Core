...
namespace rythe::core::assets
{
    ...
    inline common::result<asset<AssetType>> AssetCache<AssetType>::createInternal(rsl::id_type nameHash, Args && ...args)
    {
    ...
    {
        std::vector<rsl::id_type> toDestroy;
    ...
    template<typename AssetType>
    inline L_ALWAYS_INLINE const detail::asset_info& AssetCache<AssetType>::info(rsl::id_type nameHash)
    {
        return instance.m_info.at(nameHash);
    }

    template<typename AssetType>
    inline common::result<asset<AssetType>> AssetCache<AssetType>::retryLoad(rsl::id_type previousLoader, rsl::id_type nameHash, const std::string& name, const fs::view& file, const AssetCache<AssetType>::import_cfg& settings)
    {
        loader_type* loader = nullptr;
        rsl::size_type loaderId;

        for (rsl::size_type i = previousLoader - 2; i != static_cast<rsl::size_type>(-1); i--)
        ...
        return rythe_exception;
    }

    template<typename AssetType>
    inline common::result<asset<AssetType>> AssetCache<AssetType>::retryLoadAsync(rsl::id_type previousLoader, rsl::id_type nameHash, const std::string& name, const fs::view& file, const import_cfg& settings, AssetCache<AssetType>::progress_type& progress)
    {
        loader_type* loader = nullptr;
        rsl::size_type loaderId;
    ...
    template<typename AssetType>
    inline void AssetCache<AssetType>::asyncLoadJob(rsl::id_type nameHash, const std::string& name, const fs::view& file, const AssetCache<AssetType>::import_cfg& settings, const std::shared_ptr<typename AssetCache<AssetType>::progress_type>& progress)
    {
        loader_type* loader = nullptr;
        rsl::size_type loaderId;

        for (rsl::size_type i = instance.m_loaders.size() - 1; i != static_cast<rsl::size_type>(-1); i--)
            if (instance.m_loaders[i]->canLoad(file))
            {
                loader = instance.m_loaders[i].get();
                loaderId = i + 1;
        ...
        progress->complete(rythe_exception_msg("No loader found that could load file"));
        return;
    }
    ...
    template<typename AssetType>
    template<typename LoaderType, typename ...Arguments>
    inline asset<AssetType> AssetCache<AssetType>::createAsLoader(rsl::id_type nameHash, const std::string& name, const std::string& path, Arguments && ...args)
    {
    ...
    template<typename AssetType>
    inline L_ALWAYS_INLINE common::result<asset<AssetType>> AssetCache<AssetType>::load(rsl::id_type nameHash, const std::string& name, const fs::view& file)
    {
        return load(nameHash, name, file, import_settings<AssetType>{});
    }

    template<typename AssetType>
    inline common::result<asset<AssetType>> AssetCache<AssetType>::load(rsl::id_type nameHash, const std::string& name, const fs::view& file, const AssetCache<AssetType>::import_cfg& settings)
    {
        auto traits = file.file_info();
        if (!traits.is_valid_path)
        {
            return rythe_exception_msg("invalid file traits: not a valid path");
        }
        else if (!traits.exists)
        {
            return rythe_exception_msg("invalid file traits: file does not exist");
        }
        else if (!traits.can_be_read)
        {
            return rythe_exception_msg("invalid file traits: file cannot be read");
        }
        ...
        return rythe_exception_msg("No loader found that could load file");
    }
    ...
    inline async::async_operation<common::result<asset<AssetType>>> AssetCache<AssetType>::loadAsync(const fs::view& file)
    {
        ...
        if (!result)
        {
            ...
            progress->complete(rythe_exception_msg(result.error().what()), result.warnings());
    ...
    template<typename AssetType>
    inline L_ALWAYS_INLINE async::async_operation<common::result<asset<AssetType>>> AssetCache<AssetType>::loadAsync(rsl::id_type nameHash, const std::string& name, const fs::view& file)
    {
        return loadAsync(nameHash, name, file, import_settings<AssetType>{});
    }

    template<typename AssetType>
    inline async::async_operation<common::result<asset<AssetType>>> AssetCache<AssetType>::loadAsync(rsl::id_type nameHash, const std::string& name, const fs::view& file, const AssetCache<AssetType>::import_cfg& settings)
    {
        ...
        if (!traits.is_valid_path)
        {
            progress->complete(rythe_exception_msg("invalid file traits: not a valid path"));
            return {};
        }
        else if (!traits.exists)
        {
            progress->complete(rythe_exception_msg("invalid file traits: file does not exist"));
            return {};
        }
        else if (!traits.can_be_read)
        {
            progress->complete(rythe_exception_msg("invalid file traits: file cannot be read"));
            return {};
        }
    ...
    template<typename AssetType>
    inline L_ALWAYS_INLINE asset<AssetType> AssetCache<AssetType>::create(rsl::id_type nameHash, const std::string& name)
    {
        return create(nameHash, name, std::string(""));
    }

    template<typename AssetType>
    inline asset<AssetType> AssetCache<AssetType>::create(rsl::id_type nameHash, const std::string& name, const std::string& path)
    {
    ...
    template<typename AssetType>
    inline L_ALWAYS_INLINE asset<AssetType> AssetCache<AssetType>::create(rsl::id_type nameHash, const std::string& name, const AssetType& src)
    {
        return create(nameHash, name, std::string(""), src);
    }

    template<typename AssetType>
    inline asset<AssetType> AssetCache<AssetType>::create(rsl::id_type nameHash, const std::string& name, const std::string& path, const AssetType& src)
    {
    ...
    template<typename AssetType>
    template<typename... Arguments>
    inline L_ALWAYS_INLINE asset<AssetType> AssetCache<AssetType>::create(rsl::id_type nameHash, const std::string& name, Arguments&&... args)
    {
    ...
    template<typename AssetType>
    template<typename ...Arguments>
    inline asset<AssetType> AssetCache<AssetType>::createWithPath(rsl::id_type nameHash, const std::string& name, const std::string& path, Arguments && ...args)
    {
        static_assert(std::is_constructible_v<AssetType, Arguments...>, "Asset type is not constructible with given argument types.");
    ...
    template<typename AssetType>
    template<typename... Args>
    inline L_ALWAYS_INLINE asset<AssetType> AssetLoader<AssetType>::create(rsl::id_type nameHash, Args&&...args) const
    {
    ...
    template<typename AssetType>
    inline bool AssetCache<AssetType>::has(rsl::id_type nameHash)
    {
    ...
    template<typename AssetType>
    inline asset<AssetType> AssetCache<AssetType>::get(rsl::id_type nameHash)
    {
        if (has(nameHash))
    ...
    template<typename AssetType>
    inline void AssetCache<AssetType>::destroy(rsl::id_type nameHash)
    {
    ...
    template<typename AssetType>
    inline L_ALWAYS_INLINE rsl::id_type asset<AssetType>::id() const noexcept
    {
        return m_id;
    }
    ...
    template<typename AssetType>
    inline L_ALWAYS_INLINE asset<AssetType> asset<AssetType>::copy(rsl::id_type nameHash, const std::string& name) const
    {
