...
namespace fmt
{
    ...
    struct formatter<rythe::core::exception>
    {
        ...
        template <typename FormatContext>
        auto format(const rythe::core::exception& error, FormatContext& ctx)
        {
    ...
    template <>
    struct formatter<rythe::core::fs_error>
    {
        ...
        template <typename FormatContext>
        auto format(const rythe::core::fs_error& error, FormatContext& ctx)
        {
            return format_to(ctx.out(), "[{}({}) T {}(...)] {}", error.file(), error.line(), error.func(), error.what());
        }
    };
    ...
    template <>
    struct formatter<rythe::core::math::vec2> {
        ...
        template <typename FormatContext>
        auto format(const rythe::core::math::vec2& p, FormatContext& ctx) {
    ...
    template <>
    struct formatter<rythe::core::math::ivec2> {
        ...
        template <typename FormatContext>
        auto format(const rythe::core::math::ivec2& p, FormatContext& ctx) {
            return format_to(
                ctx.out(),
                "({}, {})",
                p.x, p.y);
    ...
    template <>
    struct formatter<rythe::core::math::vec3> {
        char presentation = 'f';
        ...
        template <typename FormatContext>
        auto format(const rythe::core::math::vec3& p, FormatContext& ctx) {
            return format_to(
                ctx.out(),
    ...
    template <>
    struct formatter<rythe::core::math::ivec3> {
        ...
        template <typename FormatContext>
        auto format(const rythe::core::math::ivec3& p, FormatContext& ctx) {
            return format_to(
                ctx.out(),
                "({}, {}, {})",
                p.x, p.y, p.z);
    ...
    struct formatter<rythe::core::math::color> {
        ...
        template <typename FormatContext>
        auto format(const rythe::core::math::color& p, FormatContext& ctx) {
            return format_to(
                ctx.out(),
                presentation == 'f' ? "({:.1f}, {:.1f}, {:.1f}, {:.1f})" : "({:.1e}, {:.1e}, {:.1e}, {:.1e})",
                p.r, p.g, p.b, p.a);
            ...
            if (!thread_ident)
            {
                ...
                else
                {
                    ...
                    *thread_ident = oss.str();
                    ...
                    //thread_ident = std::to_string(rythe::core::force_value_cast<rsl::uint>(std::this_thread::get_id()));
                }
            }
