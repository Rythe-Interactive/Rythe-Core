...
namespace rythe::core::filesystem
{
	...
	public:
		 ...
		 */
		explicit basic_resource(std::nullptr_t placeholder)
			: m_container{}
		{
		}
		 ...
		 */
		explicit basic_resource(rsl::byte_vec v)
		 ...
		 */
		explicit basic_resource(std::string_view v)
			: basic_resource(nullptr)
		{
	...
	template <typename T, typename C1 = std::enable_if<detail::has_to_resource<T, void(basic_resource*, const T&)>::value>>
	...
	template <typename T, typename C1 = std::enable_if<detail::has_from_resource<T, void(T*, const basic_resource&)>::value>>
	void from_resource(T* value, const basic_resource& resource)
	{
		T::from_resource(value, resource);
	}

	template <typename T, typename C1 = std::enable_if<detail::has_from_resource<T, void(T*, const basic_resource&)>::value>, typename C2 = std::enable_if<std::is_default_constructible<T>::value>, typename C3 = std::enable_if<std::is_move_constructible<T>::value>>
	...
	template <typename T, typename C1 = std::enable_if<detail::has_from_resource<T, void(T*, const basic_resource&)>::value>, typename C2 = std::enable_if<std::is_move_constructible<T>::value>, class... Args>
	...
	namespace literals
	{
		 ...
		 */
		inline basic_resource operator""_res(const char* str, rsl::size_type len)
		{
