...
namespace rythe::core::filesystem
{
    ...
    public:
        ...
        {
            if(!exists()) return rythe_fs_error("file does not exist, cannot read");
            if(!is_file()) return rythe_fs_error("not a file");
            if(!readable()) return rythe_fs_error("file not readable");
        ...
        common::result<void,fs_error> set(interfaces::implement_signal_t, const basic_resource& res) override
        {
            ...
            if(!writeable()) return rythe_fs_error("file not writeable");
            if(is_directory())
            {
                if(!res.empty()) return rythe_fs_error("attempted to create directory with data!");
                std::error_code code;
