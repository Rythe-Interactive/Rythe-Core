...
namespace rythe::core::filesystem
{
	...
	{
	public:
		explicit basic_resolver(std::string_view view)
			: m_root_path(view)
		{
		...
		[[nodiscard]] bool is_directory() const noexcept override
		{
			// const auto back = get_target().back();

			return (get_target().back() == '\\' || get_target().back() == '/') && is_valid_path() && !std::filesystem::is_regular_file(strpath_manip::subdir(m_root_path, get_target()));
		}
		...
		[[nodiscard]] bool is_valid_path() const noexcept override
		{
			...
			if (full.find_first_of("\x00\x01\x02\x03\x04\x05\x06\x07\x09\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x20\x21\x22\x23\x24\x25\x26\x27\x28\x29\x30\x31") != std::string::npos)
				return false; // no characters that don't exists on the keyboard pls


			return true;
		}

		[[nodiscard]] bool writeable() const noexcept override
		{
			if (!is_valid_path())
				return false;
		...
		[[nodiscard]] bool creatable() const noexcept override
		{
		...
		[[nodiscard]] std::set<std::string> ls() const noexcept override
		{
		...
		{
			if (!exists())
				return rythe_fs_error("file does not exist, cannot read");
			if (!is_file())
				return rythe_fs_error("not a file");
			if (!readable())
				return rythe_fs_error("file not readable");
		...
		common::result<void, fs_error> set(interfaces::implement_signal_t, const basic_resource& res) override
		{
			...
			if (!writeable())
				return rythe_fs_error("file not writeable");
			if (is_directory())
			{
				if (!res.empty())
					return rythe_fs_error("attempted to create directory with data!");
				std::error_code code;
				std::filesystem::create_directories(full, code);
				if (code.value() != 0)
				{
					return rythe_fs_error(("std::filesystem bailed! " + code.message()).c_str());
				}
