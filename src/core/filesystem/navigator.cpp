...
namespace rythe::core::filesystem {
    common::result<navigator::solution,fs_error> navigator::find_solution(const std::string& opt_root_domain) const 
    {
        ...
        if (!opt_root_domain.empty())
        {
            ...
            if (rdIndex == std::string::npos) return rythe_fs_error("invalid syntax for path string, no domain delimiter");
        ...
        if(root_domain.empty()) return rythe_fs_error("invalid syntax for path string, one or more properties empty");
        root_domain += std::string(":") + strpath_manip::separator() + strpath_manip::separator();
        if(!provider_registry::has_domain(root_domain)) return rythe_fs_error(("no start! no such domain: " + root_domain).c_str());
        ...
        for (auto& token : tokens) {
            ...
            if(previous_domain != domain)
            {
                ...
                if(!provider_registry::has_domain(domain)) return rythe_fs_error(("stop! no such domain: " + domain).c_str());
                ...
                //get new resolver
                resolver = *provider_registry::domain_get_first_resolver(domain);
                if(!dynamic_cast<memory_resolver_common_base*>(resolver)) return rythe_fs_error("sub domain resolver was not a mem resolver, illegal access");
