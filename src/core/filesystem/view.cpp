...
namespace rythe::core::filesystem
{
    ...
    common::result<basic_resource, fs_error> view::get()
    {
        ...
        auto resolver = build();
        if (resolver == nullptr) return rythe_fs_error("unable to get required filesystem to get resource!");
    ...
    common::result<void, fs_error> view::set(const basic_resource& resource)
    {
        ...
        //get resolver of solution
        auto resolver = build();
        if (resolver == nullptr) return rythe_fs_error("unable to get required filesystem to set resource!");
