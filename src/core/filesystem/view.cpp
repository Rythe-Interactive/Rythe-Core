...
namespace rythe::core::filesystem
{
	...
	bool view::is_valid(bool deep_check) const
	{
		...
		if (m_path.empty())
			return false;
		if (!provider_registry::has_domain(get_domain()))
			return false;
	...
	common::result<basic_resource, fs_error> view::get()
	{
		...
		auto resolver = build();
		if (resolver == nullptr)
			return rythe_fs_error("unable to get required filesystem to get resource!");

		// get & check traits
		const auto traits = resolver->get_traits();
		if (!traits.is_valid_path)
		{
			return rythe_fs_error("invalid file traits: not a valid path");
		}
		else if (!traits.exists)
		{
			return rythe_fs_error("invalid file traits: file does not exist");
		}
		else if (!traits.can_be_read)
		{
			return rythe_fs_error("invalid file traits: file cannot be read");
		}
	...
	common::result<void, fs_error> view::set(const basic_resource& resource)
	{
		...
		auto resolver = build();
		if (resolver == nullptr)
			return rythe_fs_error("unable to get required filesystem to set resource!");
		...
		return rythe_fs_error("invalid file traits: (not valid) or (not writeable or directory) or (not creatable)");
	}
	...
	common::result<std::vector<view>, fs_error> view::ls() const
	{
		...
		return rythe_fs_error("invalid file traits: (not valid) or (does not exist) or (cannot be read)");
	}
