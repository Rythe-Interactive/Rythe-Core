...
{
    template<>
    struct hash<rythe::core::detail::vertex_hash>
    {
        size_t operator()(rythe::core::detail::vertex_hash const& vh) const
        {
            return vh.hash;
        }
    };
...
namespace rythe::core
{
    ...
    common::result<asset_ptr> ObjMeshLoader::loadImpl(rsl::id_type nameHash, const fs::view& file, const import_cfg& settings, progress_type* progress)
    {
        // tinyobj objects
        ...
        {
            return { rythe_fs_error(reader.Error().c_str()), warnings };
        }
        ...
        std::unordered_map<detail::vertex_hash, rsl::uint> indices;
        ...
        for (auto& shape : shapes)
        {
            ...
            for (auto& indexData : shape.mesh.indices)
            {
                ...
                const rsl::uint vertexIndex = static_cast<rsl::uint>((vertexCount + (indexData.vertex_index * 3)) % vertexCount);

                if (vertexIndex + 3 > attributes.vertices.size())
                    continue;

                const int normalCount = static_cast<int>(attributes.normals.size());
                const rsl::uint normalIndex = static_cast<rsl::uint>((normalCount + (indexData.normal_index * 3)) % normalCount);
                ...
                if (indices[hash] >= vertices.size() || vertices[indices[hash]] != hash)
                {
                    ...
                    indices[hash] = static_cast<rsl::uint>(vertices.size());
        ...
        if (convertWinding)
            for (rsl::size_type i = 0; i < data.indices.size(); i += 3)
            {
                rsl::uint i1 = data.indices[i + 1];
                rsl::uint i2 = data.indices[i + 2];
                data.indices[i + 1] = i2;
                data.indices[i + 2] = i1;
            }
    ...
    common::result<asset_ptr> ObjMeshLoader::load(rsl::id_type nameHash, const fs::view& file, const import_cfg& settings)
    {
        return loadImpl(nameHash, file, settings, nullptr);
    }

    common::result<asset_ptr> ObjMeshLoader::loadAsync(rsl::id_type nameHash, const fs::view& file, const import_cfg& settings, progress_type& progress)
    {
