...
namespace rythe::core
{
	namespace detail
	{
		static assets::asset<image> loadGLTFImage(const tinygltf::Image& img)
		{
			...
			return assets::AssetCache<image>::createAsLoader<GltfFauxImageLoader>(hash, img.name, "",
																				  ...
																				  math::int2(img.width, img.height), img.pixel_type == TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE ? channel_format::eight_bit : img.pixel_type == TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT ? channel_format::sixteen_bit
																																																																	: channel_format::float_hdr,
																				  img.component == 1 ? image_components::grey : img.component == 2 ? image_components::grey_alpha
																															: img.component == 3   ? image_components::rgb
																																				   : image_components::rgba,
		...
		static void handleGltfBuffer(const tinygltf::Model& model, const tinygltf::Accessor& accessor, std::vector<T>& data)
		{
			...
			const rsl::size_type bufferStart = bufferView.byteOffset + accessor.byteOffset;
			const rsl::size_type stride = static_cast<rsl::size_type>(accessor.ByteStride(bufferView));
			const rsl::size_type bufferEnd = bufferStart + accessor.count * stride;

			const rsl::size_type dataStart = data.size();
			...
			if (accessor.sparse.isSparse)
			{
				...
				const auto& indexView = model.bufferViews.at(static_cast<rsl::size_type>(indices.bufferView));
				const auto& indexBuffer = model.buffers.at(static_cast<rsl::size_type>(indexView.buffer));
				const rsl::size_type indexStart = indexView.byteOffset + static_cast<rsl::size_type>(indices.byteOffset);
				const rsl::size_type indexStride = (indexView.byteStride == 0 ? sizeof(rsl::uint16) : indexView.byteStride);

				const auto& valueView = model.bufferViews.at(static_cast<rsl::size_type>(values.bufferView));
				const auto& valueBuffer = model.buffers.at(static_cast<rsl::size_type>(valueView.buffer));
				const rsl::size_type valueStart = valueView.byteOffset + static_cast<rsl::size_type>(values.byteOffset);
				const rsl::size_type valueStride = (valueView.byteStride == 0 ? sizeof(T) : valueView.byteStride);

				rsl::size_type indexPos = indexStart;
				rsl::size_type valuePos = valueStart;
		...
		static void handleGltfBuffer(const tinygltf::Model& model, const tinygltf::Accessor& accessor, std::vector<rsl::math::float3>& data, const math::float4x4 transform, bool normal = false)
		{
			...
			if (accessor.sparse.isSparse)
			{
				...
				const auto& valueBuffer = model.buffers.at(static_cast<rsl::size_type>(valueView.buffer));
				const rsl::size_type valueStart = valueView.byteOffset + static_cast<rsl::size_type>(values.byteOffset);
				const rsl::size_type valueStride = (valueView.byteStride == 0 ? 3 * sizeof(float) : valueView.byteStride);
		...
		static common::result<void, void> handleGltfVertexColor(const tinygltf::Model& model, const tinygltf::Accessor& accessor, int accessorType, int componentType, std::vector<math::color>& data)
		{
			...
			if (accessor.sparse.isSparse)
			{
				...
				const auto& valueBuffer = model.buffers.at(static_cast<rsl::size_type>(valueView.buffer));
				const rsl::size_type valueStart = valueView.byteOffset + static_cast<rsl::size_type>(values.byteOffset);
				const rsl::size_type valueStride = (valueView.byteStride == 0 ? sizeof(rsl::uint16) : valueView.byteStride);
		 ...
		 */
		static common::result<void, void> handleGltfIndices(const tinygltf::Model& model, const tinygltf::Accessor& accessor, rsl::size_type offset, std::vector<rsl::uint>& data)
		{
			const tinygltf::BufferView& bufferView = model.bufferViews.at(static_cast<rsl::size_type>(accessor.bufferView));
			const tinygltf::Buffer& buffer = model.buffers.at(static_cast<rsl::size_type>(bufferView.buffer));
			const rsl::size_type bufferStart = bufferView.byteOffset + accessor.byteOffset;
			...
			switch (accessor.componentType)
			{
				...
				{
					for (rsl::size_type i = bufferStart; i < bufferEnd; i += stride)
						data.push_back(static_cast<rsl::uint>(buffer.data[i] + offset));
				}
				break;
				case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
				{
					for (rsl::size_type i = bufferStart; i < bufferEnd; i += stride)
						data.push_back(static_cast<rsl::uint>(*reinterpret_cast<const rsl::uint16*>(&buffer.data[i]) + offset));
				}
				break;
				case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT:
				{
					for (rsl::size_type i = bufferStart; i < bufferEnd; i += stride)
						data.push_back(*reinterpret_cast<const rsl::uint*>(&buffer.data[i]) + static_cast<rsl::uint>(offset));
				}
				break;
				default:
				...
				for (int i = 0; i < sparse.count; i++)
				{
					switch (accessor.componentType)
					{
						case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:
						{
							...
							data.at(dataStart + *idx) = static_cast<rsl::uint>(valueBuffer.data[valuePos] + offset);
						}
						break;
						case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
						{
							...
							data.at(dataStart + *idx) = static_cast<rsl::uint>(*reinterpret_cast<const rsl::uint16*>(&valueBuffer.data[valuePos]) + offset);
						}
						break;
						case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT:
						{
							...
							data.at(dataStart + *idx) = *reinterpret_cast<const rsl::uint*>(&valueBuffer.data[valuePos]) + static_cast<rsl::uint>(offset);
						}
						break;
					}
		...
		static common::result<void, void> handleGltfMesh(mesh& meshData, const tinygltf::Model& model, const tinygltf::Mesh& mesh, const math::float4x4& transform)
		{
			...
			rsl::size_type primitiveIndex = 0;
			for (auto primitive : mesh.primitives)
			{
				...
				const rsl::size_type vertexOffset = meshData.vertices.size();
				...
				for (auto& attrib : primitive.attributes)
				{
					...
					const tinygltf::Accessor& accessor = model.accessors.at(static_cast<rsl::size_type>(attrib.second));
				...
				const rsl::size_type smallestBufferSize = std::min(meshData.normals.size(), std::min(meshData.uvs.size(), meshData.colors.size()));
				const rsl::size_type vertexCount = meshData.vertices.size();
				...
				const rsl::size_type index = static_cast<rsl::size_type>(primitive.indices);
				...
				{
					for (rsl::uint i = static_cast<rsl::uint>(vertexOffset); i < meshData.vertices.size(); i++)
		...
		static common::result<void, void> handleGltfNode(mesh& meshData, const tinygltf::Model& model, const tinygltf::Node& node, const math::float4x4& parentTransf)
		{
			...
			const rsl::size_type meshIdx = static_cast<rsl::size_type>(node.mesh);
			...
			{
				const rsl::size_type idx = static_cast<rsl::size_type>(nodeIdx);
	...
	common::result<asset_ptr> GltfMeshLoader::loadImpl(rsl::id_type nameHash, const fs::view& file, const import_cfg& settings, progress_type* progress)
	{
		...
		warnings.insert(warnings.end(), result.warnings().begin(), result.warnings().end());
		if (!result)
			return {rythe_exception_msg(result.error().what()), warnings};

		auto extension = file.get_extension();
		if (!extension)
			return {rythe_exception_msg(extension.error().what()), warnings};
		...
		else if (extension.value() == ".glb")
		{
			...
			return {rythe_exception_msg("File was not recognised as a gltf file."), warnings};
		...
		std::unordered_map<std::string, std::pair<bool, rsl::size_type>> materialNames;
		...
		for (auto& srcMat : model.materials)
		{
			...
			if (materialNames.count(material.name))
				...
				materialNames.emplace(material.name, std::make_pair<bool, rsl::size_type>(false, 0u));
			...
			if (pbrData.baseColorTexture.index >= 0)
				material.albedoMap = detail::loadGLTFImage(
					model.images[static_cast<rsl::size_type>(model.textures[static_cast<rsl::size_type>(pbrData.baseColorTexture.index)].source)]
				);
			...
			if (pbrData.metallicRoughnessTexture.index >= 0)
				material.metallicRoughnessMap = detail::loadGLTFImage(
					model.images[static_cast<rsl::size_type>(model.textures[static_cast<rsl::size_type>(pbrData.metallicRoughnessTexture.index)].source)]
				);
			...
			if (srcMat.emissiveTexture.index >= 0)
				material.emissiveMap = detail::loadGLTFImage(
					model.images[static_cast<rsl::size_type>(model.textures[static_cast<rsl::size_type>(srcMat.emissiveTexture.index)].source)]
				);

			if (srcMat.normalTexture.index >= 0)
				material.normalMap = detail::loadGLTFImage(
					model.images[static_cast<rsl::size_type>(model.textures[static_cast<rsl::size_type>(srcMat.normalTexture.index)].source)]
				);

			if (srcMat.occlusionTexture.index >= 0)
				material.aoMap = detail::loadGLTFImage(
					model.images[static_cast<rsl::size_type>(model.textures[static_cast<rsl::size_type>(srcMat.occlusionTexture.index)].source)]
				);
		...
		{
			return {rythe_exception_msg("GLTF model contained 0 scenes"), warnings};
		}

		const rsl::size_type sceneToLoad = model.defaultScene < 0 ? 0 : static_cast<rsl::size_type>(model.defaultScene);
		...
		if (convertWinding)
			for (rsl::size_type i = 0; i < meshData.indices.size(); i += 3)
			{
				rsl::uint i1 = meshData.indices[i + 1];
				rsl::uint i2 = meshData.indices[i + 2];
	...
	common::result<asset_ptr> GltfMeshLoader::load(rsl::id_type nameHash, const fs::view& file, const import_cfg& settings)
	{
		return loadImpl(nameHash, file, settings, nullptr);
	}

	common::result<asset_ptr> GltfMeshLoader::loadAsync(rsl::id_type nameHash, const fs::view& file, const import_cfg& settings, progress_type& progress)
	{
