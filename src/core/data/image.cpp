...
namespace rythe::core
{
    ...
    {
        m_colors->reserve(static_cast<rsl::size_type>(m_resolution.x * m_resolution.y));
        ...
        for (auto colorPtr = m_data.begin(); colorPtr < end; colorPtr += colorSize)
        {
            ...
            case image_components::grey:
            {
                ...
                {
                    float grayValue = (*reinterpret_cast<rsl::uint16*>(colorPtr) / 65535.f);
                    color.r = grayValue;
                    color.g = grayValue;
                    color.b = grayValue;
                    color.a = 1.f;
                ...
                default:
                    return rythe_exception_msg("invalid channel format");
                }
                break;
            }
            case image_components::grey_alpha:
            {
                ...
                case channel_format::sixteen_bit:
                {
                    float grayValue = (*reinterpret_cast<rsl::uint16*>(gPtr) / 65535.f);
                    color.r = grayValue;
                    color.g = grayValue;
                    color.b = grayValue;
                    color.a = (*reinterpret_cast<rsl::uint16*>(aPtr) / 65535.f);
                    break;
                }
                case channel_format::float_hdr:
                {
            ...
            case image_components::rgb:
            {
                ...
                case channel_format::sixteen_bit:
                {
                    color.r = (*reinterpret_cast<rsl::uint16*>(rPtr) / 65535.f);
                    color.g = (*reinterpret_cast<rsl::uint16*>(gPtr) / 65535.f);
                    color.b = (*reinterpret_cast<rsl::uint16*>(bPtr) / 65535.f);
                    color.a = 1.f;
                    break;
                }
                case channel_format::float_hdr:
            ...
            default:
                return rythe_exception_msg("invalid image components");
            }
    ...
    {
        auto channelSize = static_cast<rsl::size_type>(m_format);
        auto colorSize = static_cast<rsl::size_type>(m_components) * channelSize;
        ...
        for (auto colorPtr = m_data.begin(); colorPtr < end; colorPtr += colorSize)
        {
            ...
            case image_components::grey:
            {
                ...
                case channel_format::sixteen_bit:
                {
                    *reinterpret_cast<rsl::uint16*>(colorPtr) = static_cast<rsl::uint16>(math::iround(colors[i].r * 65535.f));
                    break;
                }
                case channel_format::float_hdr:
                {
            ...
            case image_components::grey_alpha:
            {
                ...
                case channel_format::sixteen_bit:
                {
                    *reinterpret_cast<rsl::uint16*>(gPtr) = static_cast<rsl::uint16>(math::iround(colors[i].r * 65535.f));
                    *reinterpret_cast<rsl::uint16*>(aPtr) = static_cast<rsl::uint16>(math::iround(colors[i].a * 65535.f));
                    break;
                }
                case channel_format::float_hdr:
                {
            ...
            case image_components::rgb:
            {
                ...
                case channel_format::sixteen_bit:
                {
                    *reinterpret_cast<rsl::uint16*>(rPtr) = static_cast<rsl::uint16>(math::iround(colors[i].r * 65535.f));
                    *reinterpret_cast<rsl::uint16*>(gPtr) = static_cast<rsl::uint16>(math::iround(colors[i].g * 65535.f));
                    *reinterpret_cast<rsl::uint16*>(bPtr) = static_cast<rsl::uint16>(math::iround(colors[i].b * 65535.f));
                    break;
                }
                case channel_format::float_hdr:
                {
    ...
    rsl::size_type image::data_size() const
    {
        return m_data.size();
    }
