...
namespace rythe::core
{
    ...
    {
        m_hook = m_context->find_process_hook({ .chain = handle, .type = process_hook_type::after });
        return *this;
    }

    inline process_chain_builder& process_chain_builder::after(rsl::string_view processChainName)
    {
        m_hook = m_context->find_process_hook(
            {
                .chain = m_context->find_process_chain(processChainName),
    ...
    {
        m_hook = m_context->find_process_hook({ .chain = handle, .type = process_hook_type::before });
        return *this;
    }
    ...
    {
        m_hook = m_context->find_process_hook({ .componentType = rsl::type_id<ComponentType>(), .type = process_hook_type::construct });
        return *this;
    }
    ...
    {
        m_hook = m_context->find_process_hook({ .componentType = rsl::type_id<ComponentType>(), .type = process_hook_type::destroy });
        return *this;
    }
