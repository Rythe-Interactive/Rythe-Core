...
namespace rythe::core
{
	...
	{
		rotation()
			: rsl::math::quat(1, 0, 0, 0)
		{
		}
		rotation(float w, float x, float y, float z)
			: rsl::math::quat(w, x, y, z)
		{
		}
	...
	struct velocity : public rsl::math::float3
	{
		...
		velocity(const rsl::math::float3& src)
			: rsl::math::float3(src)
		{
		}
	...
	struct mesh_filter
	{
		...
		explicit mesh_filter(const assets::asset<mesh>& src)
			: shared_mesh(src)
		{
		}
...
namespace std // NOLINT(cert-dcl58-cpp)
{
	...
	template <>
	struct tuple_size<rythe::core::transform> : public std::integral_constant<std::size_t, 3>
	{
	};

	template <>
	struct tuple_size<const rythe::core::transform> : public std::integral_constant<std::size_t, 3>
	{
	};

	template <>
	struct tuple_size<volatile rythe::core::transform> : public std::integral_constant<std::size_t, 3>
	{
	};

	template <>
	struct tuple_size<const volatile rythe::core::transform> : public std::integral_constant<std::size_t, 3>
	{
	};
