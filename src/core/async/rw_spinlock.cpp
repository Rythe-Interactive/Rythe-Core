...
namespace rythe::core::async
{
    ...
    {
        static thread_local std::unordered_map<rsl::uint, int> m_localWriters;
        return m_localWriters;
    }

    std::unordered_map<rsl::uint, int>& rw_spinlock::localReaders()
    {
       static thread_local std::unordered_map<rsl::uint, int> m_localReaders;
       return m_localReaders;
    }

    std::unordered_map<rsl::uint, lock_state>& rw_spinlock::localState()
    {
        static thread_local std::unordered_map<rsl::uint, lock_state> m_localState;
        return m_localState;
    }
    ...
    void rw_spinlock::lock(lock_state permissionLevel, wait_priority priority) const
    {
        ...
        case rythe::core::async::lock_state::idle:
        default:
            return;
        }
    }
    ...
    void rw_spinlock::unlock(lock_state permissionLevel) const
    {
        ...
        switch (permissionLevel)
        {
        case rythe::core::async::lock_state::read:
            return read_unlock();
        case rythe::core::async::lock_state::write:
            return write_unlock();
        case rythe::core::async::lock_state::idle:
        default:
            return;
