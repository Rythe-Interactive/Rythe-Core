...
namespace rythe::core::async
{
	 ...
	 * @ref rythe::core::async::readonly_guard
	 * @ref rythe::core::async::readwrite_guard
	 * @ref rythe::core::async::readonly_multiguard
	 * @ref rythe::core::async::readwrite_multiguard
	 * @ref rythe::core::async::mixed_multiguard
	 */
	...
	private:
		...
		static std::unordered_map<rsl::uint, lock_state>& localState();
	 ...
	 * @note Read-only operations can happen simultaneously without waiting for each other.
	 *       Read-only operations will only wait for Read-Write operations to be finished.
	 * @ref rythe::core::async::rw_spinlock
	 */
	...
	public:
		 ...
		 */
		readonly_guard(const rw_spinlock& lock, wait_priority priority = wait_priority::real_time)
			: m_lock(lock)
		{
	...
	public:
		...
		readonly_multiguard(const lock_type1& lock1, const lock_type2& lock2, const lock_typesN&... locks)
			: m_locks{
				  {&lock1, &lock2, &locks...}
        }
		{
	...
	public:
		 ...
		 */
		readwrite_guard(const rw_spinlock& lock, wait_priority priority = wait_priority::real_time)
			: m_lock(lock)
		{
	...
	public:
		...
		readwrite_multiguard(const lock_type1& lock1, const lock_type2& lock2, const lock_typesN&... locks)
			: m_locks{
				  {&lock1, &lock2, &locks...}
        }
		{
