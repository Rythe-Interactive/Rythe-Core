...
namespace rythe::core::async
{
    ...
    public:
        ring_sync_lock(rsl::uint maxRank = UINT32_MAX) : m_maxRank(maxRank), owningThread(std::this_thread::get_id()) { m_release.store(false, std::memory_order_relaxed); }
        ...
        void sync()
        {
            rsl::uint rank = m_rank.load(std::memory_order_acquire); // Fetch the current rank at which the lock should be.
            rsl::uint nextRank = (rank + 1) % m_maxRank; // Calculate the next rank at which the lock should be.
