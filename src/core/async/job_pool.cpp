...
{
    thread_local rsl::id_type this_job::m_id;
    thread_local float this_job::m_progress;

    rsl::id_type this_job::get_id() noexcept
    {
        return m_id;
    }
    ...
    {
        rsl::size_type idx = m_index.load(std::memory_order_relaxed);
        return idx < 1 || idx > m_size;
    }

    bool job_pool::prime_job()
    {
        rsl::size_type idx = m_index.fetch_sub(1, std::memory_order_acquire);
        rsl::size_type id = m_size - idx;
        this_job::m_id = id;
        return idx == 1u;
    }
