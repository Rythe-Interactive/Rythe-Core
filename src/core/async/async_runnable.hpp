...
namespace rythe::core::async
{
	struct async_runnable_base
	{
		...
		async_runnable_base(const std::shared_ptr<async_progress_base>& progress)
			: m_progress(progress)
		{
		}
	...
	{
		using payload_type = std::conditional_t<std::is_invocable_v<functor, async::async_progress_base&>, decltype(std::invoke(std::declval<functor>(), std::declval<async::async_progress_base&>())), decltype(std::invoke(std::declval<functor>()))>;
	...
	public:
		...
		async_runnable(functor&& func)
			: async_runnable_base(std::make_shared<async_progress<payload_type>>(1)),
			  m_func(func)
		{
		}
		async_runnable(functor&& func, rsl::size_type taskSize)
			: async_runnable_base(std::make_shared<async_progress<payload_type>>(taskSize)),
			  m_func(func)
		{
		}
	...
	template <typename functor>
	async_runnable(functor&&, rsl::size_type) -> async_runnable<functor>;
#endif
