...
namespace rythe::core::async
{
	...
	rsl::size_type async_progress_base::raw_size() const noexcept
	{
		return m_size;
	}

	rsl::size_type async_progress_base::raw_progress() const noexcept
	{
	...
	{
		m_progress.fetch_add(static_cast<rsl::size_type>(progress * precision_scale<float>), std::memory_order_release);
	}

	void async_progress_base::reset(float progress) noexcept
	{
		m_progress.store(static_cast<rsl::size_type>(progress * precision_scale<float>), std::memory_order_release);
	}
