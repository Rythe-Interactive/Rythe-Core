...
namespace rythe::core::async
{
	...
	public:
		...
		explicit transferable_atomic(T val)
			: m_atomic(val),
			  m_lock()
		{
		}

		transferable_atomic(const std::atomic<T>& other)
			: m_atomic(other.load(std::memory_order_acquire)),
			  m_lock()
		{
		}

		transferable_atomic(const transferable_atomic<T>& other)
			: m_atomic(other->load(std::memory_order_acquire)),
			  m_lock()
		{
		}

		transferable_atomic(std::atomic<T>&& other)
			: m_atomic(other.load(std::memory_order_acquire)),
			  m_lock()
		{
		...
		transferable_atomic(transferable_atomic<T>&& other)
			: m_atomic(other->load(std::memory_order_acquire)),
			  m_lock()
		{
			other->store(T(), std::memory_order_release);
