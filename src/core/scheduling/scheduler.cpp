...
namespace rythe::core::scheduling
{
    ...
    void Scheduler::threadMain(Engine& engine, bool lowPower, std::string name)
    {
        ...
        while (!instance.m_exit.load(std::memory_order_relaxed))
        {
            ...
            if (noWork)
            {
                ...
                if (lowPower || RYTHE_CONFIGURATION == RYTHE_DEBUG_VALUE)
                {
    ...
    int Scheduler::run(Engine& engine, bool lowPower, rsl::size_type minThreads)
    {
        hookThreadToEngine(engine);
        getOnThreadCreate()();
        ...
        rsl::size_type m_minThreads = minThreads < 1 ? 1 : minThreads;
        ...
        while (!instance.m_exit.load(std::memory_order_relaxed))
        {
            ...
            static rsl::size_type framecount = 0;
            static float totalTime = 0;
            static rsl::uint bestAvg = 0;
            ...
            if (totalTime > 5.f)
            {
                rsl::uint avg = math::uround(1.f / (totalTime / static_cast<float>(framecount)));
                totalTime = 0.f;
                framecount = 0;
    ...
    rsl::size_type Scheduler::jobPoolSize() noexcept
    {
    ...
    pointer<ProcessChain> Scheduler::getChain(rsl::id_type id)
    {
    ...
    void Scheduler::subscribeToChainStart(rsl::id_type chainId, const chain_callback_delegate& callback)
    {
    ...
    void Scheduler::unsubscribeFromChainStart(rsl::id_type chainId, const chain_callback_delegate& callback)
    {
    ...
    void Scheduler::subscribeToChainEnd(rsl::id_type chainId, const chain_callback_delegate& callback)
    {
    ...
    bool Scheduler::unhookProcess(rsl::id_type chainId, pointer<Process> process)
    {
        if (instance.m_processChains.contains(chainId))
            return instance.m_processChains[chainId].removeProcess(process);
