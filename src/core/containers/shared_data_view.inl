...
namespace rythe::core
{
	...
	template <typename DataType>
	inline DataType& shared_data_view<DataType>::at(rsl::size_type idx)
	{
		if (idx > m_size)
			throw std::out_of_range("shared_data_view subscript out of range");
		return *(m_data.get() + m_offset + static_cast<ptrdiff_t>(idx));
	}

	template <typename DataType>
	inline const DataType& shared_data_view<DataType>::at(rsl::size_type idx) const
	{
		if (idx > m_size)
			throw std::out_of_range("shared_data_view subscript out of range");
		return *(m_data.get() + m_offset + static_cast<ptrdiff_t>(idx));
	...
	template <typename DataType>
	inline DataType& shared_data_view<DataType>::operator[](rsl::size_type idx)
	{
		return at(idx);
	}

	template <typename DataType>
	inline const DataType& shared_data_view<DataType>::operator[](rsl::size_type idx) const
	{
		return at(idx);
	}
	...
	template <typename DataType>
	inline rsl::size_type shared_data_view<DataType>::size() const noexcept
	{
		return m_size;
	}
	...
	template <typename DataType>
	inline rsl::size_type shared_data_view<DataType>::max_size() const noexcept
	{
		return m_size;
	}
} // namespace rythe::core
