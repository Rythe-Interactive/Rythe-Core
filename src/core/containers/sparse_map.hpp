...
namespace rythe::core
{
    ...
    private:
        ...
        rsl::size_type m_size = 0;
        rsl::size_type m_capacity = 0;

    public:
        sparse_map() = default;
        sparse_map(rsl::size_type capacity)
        {
            reserve(capacity);
        }

        [[nodiscard]] dense_value_container& values() noexcept { return m_dense_value; }
        [[nodiscard]] const dense_value_container& values() const noexcept { return m_dense_value; }

        [[nodiscard]] dense_key_container& keys() noexcept { return m_dense_key; }
        [[nodiscard]] const dense_key_container& keys() const noexcept { return m_dense_key; }

        [[nodiscard]] iterator begin() noexcept { return iterator(m_dense_key.begin(), m_dense_value.begin()); }
        [[nodiscard]] const_iterator begin() const noexcept { return const_iterator(m_dense_key.cbegin(), m_dense_value.cbegin()); }
        [[nodiscard]] const_iterator cbegin() const noexcept { return const_iterator(m_dense_key.cbegin(), m_dense_value.cbegin()); }

        [[nodiscard]] iterator end() noexcept { return iterator(m_dense_key.begin() + m_size, m_dense_value.begin() + m_size); }
        [[nodiscard]] const_iterator end() const noexcept { return const_iterator(m_dense_key.cbegin() + m_size, m_dense_value.cbegin() + m_size); }
        [[nodiscard]] const_iterator cend() const noexcept { return const_iterator(m_dense_key.cbegin() + m_size, m_dense_value.cbegin() + m_size); }
         ...
         */
        [[nodiscard]] rsl::size_type max_size() const noexcept { return m_dense_value.max_size(); }
         ...
         */
        void reserve(rsl::size_type size)
        {
            if (size > m_capacity)
            {
...
#pragma region count
         ...
         */
        [[nodiscard]] rsl::size_type count(key_const_reference key) const
        {
            return contains(key);
        }
         ...
         * @ref rythe::core::sparse_map::contains
         */
        [[nodiscard]] rsl::size_type count(key_type&& key) const
        {
            return contains(key);
        }
#pragma endregion
...
#pragma region contains
         ...
         */
        [[nodiscard]] bool contains(key_const_reference key) const
        {
            if (!m_sparse.count(key))
                return false;

            const rsl::size_type& sparseval = m_sparse.at(key);
         ...
         */
        template<typename T>
        [[nodiscard]] bool contains(const sparse_map<key_type, T>& other) const
        {
            if (other.m_size == 0)
                return true;
...
#pragma endregion
         ...
         */
        rsl::size_type erase(key_const_reference key)
        {
            if (contains(key))
            {
