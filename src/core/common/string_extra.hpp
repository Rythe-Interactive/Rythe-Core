...
namespace rythe::core::common {
    ...
    constexpr inline rsl::size_type constexpr_strlen(rsl::cstring str)
    {
        return ((0 == str) ? 0 : (*str == '\0') ? 0 : 1 + constexpr_strlen((const char*)(str + 1)));
    }

    inline bool starts_with(const std::string& src, rsl::cstring value)
    {
        return src.rfind(value, 0) == 0;
    }

    inline bool ends_with(const std::string& src, rsl::cstring value)
    {
