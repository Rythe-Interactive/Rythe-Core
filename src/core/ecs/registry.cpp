...
{
	component_pool_base* Registry::tryEmplaceFamily(rsl::id_type id, std::unique_ptr<component_pool_base>&& ptr)
	{
	...
	std::unordered_map<rsl::id_type, std::unique_ptr<component_type_base>>& Registry::componentTypes()
	{
		static std::unordered_map<rsl::id_type, std::unique_ptr<component_type_base>> m_componentTypes;
		return m_componentTypes;
	}

	rsl::id_type Registry::getNextEntityId()
	{
		...
		return instance.m_recyclableEntities.empty() ?                 // If there are no recyclable entityIDs
				   instance.m_nextEntityId++
													 :                 // Then we increase the next entityID and use that.
				   []() {                                              // Lanbda to handle recycling case.
			rsl::id_type temp = instance.m_recyclableEntities.front(); // Copy out first item.
	...
	component_pool_base* Registry::getFamily(rsl::id_type typeId)
	{
		return getFamilies().at(typeId).get();
	}

	const std::string& Registry::getFamilyName(rsl::id_type id)
	{
	...
	std::unordered_map<rsl::id_type, std::unique_ptr<component_pool_base>>& Registry::getFamilies()
	{
	...
	{
		const rsl::id_type currentEntityId = getNextEntityId();
	...
	void Registry::destroyEntity(rsl::id_type target, bool recurse)
	{
	...
	bool Registry::checkEntity(rsl::id_type target)
	{
		return instance.m_entities.count(target) && instance.m_entities.at(target).alive;
	}

	std::unordered_map<rsl::id_type, std::unordered_set<rsl::id_type>>& Registry::entityCompositions()
	{
		return instance.m_entityCompositions;
	}

	std::unordered_map<rsl::id_type, entity_data>& Registry::entityData()
	{
		return instance.m_entities;
	}

	std::unordered_set<rsl::id_type>& Registry::entityComposition(entity target)
	{
		return instance.m_entityCompositions.at(target);
	}

	std::unordered_set<rsl::id_type>& Registry::entityComposition(rsl::id_type target)
	{
		return instance.m_entityCompositions.at(target);
	}

	entity_data& Registry::entityData(rsl::id_type target)
	{
		return instance.m_entities.at(target);
	}

	entity Registry::getEntity(rsl::id_type target)
	{
		return entity{&entityData(target)};
	}

	void* Registry::createComponent(rsl::id_type typeId, entity target)
	{
	...
	void Registry::destroyComponent(rsl::id_type typeId, entity target)
	{
		// Update entity composition.
	...
	bool Registry::hasComponent(rsl::id_type typeId, entity target)
	{
		return getFamily(typeId)->contains(target);
	}

	void* Registry::getComponent(rsl::id_type typeId, entity target)
	{
